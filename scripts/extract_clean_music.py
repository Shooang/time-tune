...
print("截取70年代新闻节目纯音乐片段:")
# 开场：0-7.5秒（前4秒呼号，后3.5秒是呼号尾，避免人声）
opening = load_mp3_segment(f_70s, 0, 7.5)
opening = fade(opening, 0.05, 2.0)
save_wav(opening, OUT_DIR / "opening_clean.wav")

# 间隔音乐1：27-34.5秒（纯音乐7.5秒，用户说19-25秒听到的就是这段）
bridge1 = load_mp3_segment(f_70s, 27.0, 7.5)
bridge1 = fade(bridge1, 0.5, 1.5)
save_wav(bridge1, OUT_DIR / "bridge_music_1.wav")

# 结尾音乐：44-50秒（淡出）
ending = load_mp3_segment(f_70s, 43.5, 7.0)
...
# 基于之前分析，试截取几个长段
segs = [
    ("ref_opening_0_12", 0, 12),
    ("ref_bridge_135_148", 135, 13),
]
for name, start, dur in segs:
    seg = load_mp3_segment(f_ref, start, dur)
    seg = fade(seg, 0.2, 1.5)
